...
JOIN SFE_STG_ENTERTAINMENT.STG_PARSED_DOCUMENTS parsed ON catalog.document_id = parsed.document_id
LEFT JOIN SFE_STG_ENTERTAINMENT.STG_TRANSLATED_CONTENT trans ON parsed.parsed_id = trans.parsed_id
LEFT JOIN SFE_STG_ENTERTAINMENT.STG_CLASSIFIED_DOCS classified ON parsed.parsed_id = classified.parsed_id
LEFT JOIN SFE_STG_ENTERTAINMENT.STG_EXTRACTED_ENTITIES entities ON parsed.parsed_id = entities.parsed_id
LEFT JOIN SFE_RAW_ENTERTAINMENT.DOCUMENT_PROCESSING_LOG log ON catalog.document_id = log.document_id
